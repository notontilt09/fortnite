...
#Route for GDisconnect to disconnect from login session.  Revoke's user token and reset login_session.
...
def newWeapon():
	...
	if request.method == 'POST':
		if request.form['name']:
			newWeapon = Weapon(name = request.form['name'], user_id = login_session['user_id'])
...
def editWeapon(weapon_name):
	...
	if editedWeapon.user_id != login_session['user_id']:
		return "<script>function myFunction() {alert('You are not authorized to edit this weapon.  Are you its creator?');}</script><body onload='myFunction()''>"
	if request.method == 'POST':
		if request.form['name']:
...
def deleteWeapon(weapon_name):
	...
	if deletedWeapon.user_id != login_session['user_id']:
		return "<script>function myFunction() {alert('You are not authorized to delete this weapon.  Are you its creator?);}</script><body onload='myFunction()''>"
	if request.method == 'POST':
...
def showWeaponDetail(weapon_name, weapon_detail_name):
	weapon = session.query(Weapon).filter_by(name = weapon_name).one()
	weapon_detail = session.query(WeaponDetail).filter_by(name = weapon_detail_name).one()
	if 'username' not in login_session:
		return render_template('publicWeaponDetail.html', weapon = weapon, weapon_detail = weapon_detail)
	else:
		return render_template('weaponDetail.html', weapon = weapon, weapon_detail = weapon_detail)
...
def newWeaponDetail(weapon_name):
	...
	weapon = session.query(Weapon).filter_by(name = weapon_name).one()
	if request.method == 'POST':
		new_weapon_detail = WeaponDetail(name = request.form['name'], description = request.form['description'], color = request.form['color'], damage = request.form['damage'], weapon_id = weapon.id, user_id = login_session['user_id'])
...
@app.route('/fortnite/<string:weapon_name>/list/<string:weapon_detail_name>/edit', methods = ['GET', 'POST'])
def editWeaponDetail(weapon_name, weapon_detail_name):
	if 'username' not in login_session:
		return redirect('/login')
	weapon = session.query(Weapon).filter_by(name = weapon_name).one()
	editedWeaponDetail = session.query(WeaponDetail).filter_by(name = weapon_detail_name).one()
	...
	if request.method == 'POST':
		...
		session.commit()
		return redirect(url_for('showWeaponList', weapon_name = weapon.name))
	return render_template('editWeaponDetail.html', weapon_detail = editedWeaponDetail, weapon_name = weapon.name)


#Route to delete weapon detail
@app.route('/fortnite/<string:weapon_name>/list/<string:weapon_detail_name>/delete', methods = ['GET', 'POST'])
def deleteWeaponDetail(weapon_name, weapon_detail_name):
	if 'username' not in login_session:
		return redirect('/login')
	weapon = session.query(Weapon).filter_by(name = weapon_name).one()
	deletedWeaponDetail = session.query(WeaponDetail).filter_by(name = weapon_detail_name).one()
	...
	if request.method == 'POST':
		...
		session.commit()
		return redirect(url_for('showWeaponList', weapon_name = weapon.name))
	return render_template('deleteWeaponDetail.html', weapon_detail = deletedWeaponDetail, weapon_name = weapon.name)
...
@app.route('/fortnite/<string:weapon_name>/list/<string:weapon_detail_name>/JSON')
